...
wiki ingest <file|url>        # literature note + fan-out updates to existing notes
                              ...
                              #   .pdf is text-extracted via pdf-parse; everything else read as UTF-8
...
`ingest` (`src/ingest.js`) is a different two-pass shape: pass 1 extracts JSON (`{summary, updates[]}`), pass 2 formats the summary as a `literature` note, then each `update` is applied to an existing note via a third prompt. Note targets that don't resolve to a file are skipped, not created. Sources over `INGEST_CHUNK_CHARS` (48,000 chars, ~20 pages — same budget as `lint`'s `LINT_CHUNK_CHARS` sharding) are split on paragraph boundaries (`chunkText`) before pass 1: each chunk gets its own `{summary, updates[]}` extraction call (told "part X of Y"), chunk summaries are concatenated for pass 2, and `updates[]` targeting the same note across chunks are merged into one addition before fan-out. Shorter sources take the original single-call path unchanged. See README's "Why one literature note per source?" for the rationale.

**Source file reading differs by front end.** The CLI (`bin/wiki.js`) reads a `.pdf` file's text via `pdf-parse` and everything else as UTF-8 — images and scanned/image-only PDFs aren't supported (no text to extract). The skill (`skills/wiki-ingest/SKILL.md`) instead reads the source with the host agent's own Read tool: it extracts PDF text directly (page-range chunks of ≤20 for long PDFs, concatenated), and for images it has the agent transcribe the visible content and use that as the source text — so the skill covers scanned PDFs and standalone images that the CLI's `pdf-parse`-only path cannot.
